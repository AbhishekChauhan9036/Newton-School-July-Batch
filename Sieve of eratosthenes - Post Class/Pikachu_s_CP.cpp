...
void sieve()
{
    for(int i=4;i<=1000009;i+=2) prime[i]=true;
    ...
    for(int i=1;i<=1000000;i++) v[cnt[i]].pb(i);
}
/*--------------------------------------------------------------------------------------------------------------------------*/
struct UnionFind {
    int n;
...
void Bhagwat_Geeta(){
  ...
  while(_Kuber--){
    int a,b,c;
    cin>>a>>b>>c;
    int z=upper_bound(v[c].begin(),v[c].end(),b)-upper_bound(v[c].begin(),v[c].end(),a);
    if(z<=0) cout<<-1<<cline;
    else cout<<z<<cline;
    }   
}
